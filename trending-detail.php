...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
  ...
  $error_message = "Invalid or missing trending item ID.";
}
...
if (empty($error_message)) {
  ...
  try {
    ...
    if (!$trending_item) {
      // Set error message instead of redirecting
      $error_message = "Trending item not found or not active.";
    }
  } catch (PDOException $e) {
    // Log the error and set a user-friendly error message
    error_log("Error fetching trending item: " . $e->getMessage());
    $error_message = "An error occurred while retrieving the trending item. Please try again later.";
  }
}

// Get related trending items (only if no error)
$related_items = [];
if (empty($error_message)) {
  try {
    $stmt = $conn->prepare("
    ...
    // Note: We don't set an error message here as it's not a critical failure
  }
}
...
if (empty($error_message)) {
  ...
  $page_title = "Error - NewsHub";
  $meta_description = "An error occurred while loading the trending item.";
  $meta_keywords = '';
}
?>
